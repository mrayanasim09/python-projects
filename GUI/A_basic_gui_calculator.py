...
if __name__ == "__main__":
    ...
    for i in range(4):
        for j in range(4):
            if buttons[i][j] == '%':
                button = Button(gui, text=buttons[i][j], fg='black', bg='red', font=('Arial', 20, 'bold'),
                                command=calculate_percentage, height=1, width=7)
                button.grid(row=i+2, column=j)
            else:
                button = Button(gui, text=buttons[i][j], fg='black', bg='red', font=('Arial', 20, 'bold'),
                                command=lambda x=buttons[i][j]: press(x), height=1, width=7)
                button.grid(row=i+2, column=j)

    clear_button = Button(gui, text='Clear', fg='black', bg='red', font=('Arial', 20, 'bold'),
                          command=clear, height=1, width=7)
    clear_button.grid(row=6, column=0, columnspan=2)

    equal_button = Button(gui, text='=', fg='black', bg='red', font=('Arial', 20, 'bold'),
